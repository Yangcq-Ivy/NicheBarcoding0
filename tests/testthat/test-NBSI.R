...
for (siu in 1:length(all.ref.sp)){
  ...
  if (dim(prese.env)[1]==1){
    ...
    warning ("The model may not be accurate because there is only one record of ",
             all.ref.sp[siu],"!\n")
  }
...
for(n in 1:nrow(que.vari)){
  ...
  if (length(target.spe) == 0){
    stop(paste("Please check the barcoding identification of ",
               queID,"!\n",sep=""))
    print(BSI.out)
  }
...
test_that("class testing of the output",{
  expect_equal(class(BSI.out[n,3]),"numeric")
