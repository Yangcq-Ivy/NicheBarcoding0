    ...
    if (is.null(nrow(diff)) || nrow(diff) == 0){
      ...
      for (rs in 1:length(ref.sd)){
        if (ref.sd[rs] == 0){
          ...
          ref.sd[rs]<-ref.mean[rs]/4 ### updated on 2021-08-22 11:38:26
...
if (model == "RF"){
  ...
  if (file.exists(jar)){### updated on 2021-08-21 17:04:13
    ...
    mod<-dismo::maxent(Data[,-1],Data[,1],args='outputformat=logistic')
    use<-c(rep(1,nrow(present.env)),rep(0,nrow(absent.env)))
...
test_that("verify the number of background points",{
  expect_gt(nrow(absent.env),nrow(present.env))
})

test_that("format testing on the built model",{
  expect_s3_class(mod,c("randomForest","MaxEnt"))
})

test_that("check whether use and prb are non-zero vectors",{
  expect_false(is.null(use))
  expect_false(is.null(prb))
  expect_false(all(is.na(use)))
  expect_false(all(is.na(prb)))
})
