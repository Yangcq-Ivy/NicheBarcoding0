...
if (nrow(ref.lonlat) < n.vari){
  warning("ref.lonlat must be more than 19 points!\n") ### updated on 2021-10-07 11:00:21
  warning("Generating the pseudo present points ......\n") ### updated on 2021-10-07 11:00:21
  ref.lonlat<-pseudo.present.points(ref.lonlat,n.vari,50,1)
}
...
test_that("check whether the coordinates are matched",{
  expect_equal(ncol(ref.lonlat),ncol(que.lonlat))
  expect_equal(length(ref.lonlat$Lon),length(ref.lonlat$Lat))
  expect_equal(length(que.lonlat$Lon),length(que.lonlat$Lat))
})

test_that("check whether the variables are matched",{
  expect_equal(colnames(data.var),colnames(que.var))
})

test_that("class testing of the two principal components",{
  expect_equal(class(data.pca1[1]),"numeric")
  expect_equal(class(data.pca2[1]),"numeric")
  expect_equal(class(que.pca1[1]),"numeric")
  expect_equal(class(que.pca2[1]),"numeric")
})
